// src/toolService.js
import { ZeroExApiService } from './services/zeroExApiService.js';
import { CoinGeckoApiService } from './services/coinGeckoApiService.js';
import { BlockchainService } from './services/blockchainService.js';
import { ethers } from 'ethers';

export class ToolService {
  constructor(proxyServerUrl, userPrivateKey, userAddress, coinGeckoApiKey, alchemyApiKey) {
    this.zeroExApi = new ZeroExApiService(proxyServerUrl);
  ...
  async getSwapPrice(params) {
    ...
    const result = await this.zeroExApi.getSwapPrice(params);
  ...
  async getSwapQuote(params) {
    ...
    const result = await this.zeroExApi.getSwapQuote(quoteParams);
  ...
  async executeSwap(quoteData) {
    ...
    try {
      ...
      const result = await this.blockchain.signAndBroadcastTransaction(chainId, quoteData);
  ...
  async getSupportedChains() {
    const result = await this.zeroExApi.getSupportedChains();
  ...
  async getLiquiditySources(chainId) {
    ...
    const result = await this.zeroExApi.getLiquiditySources(chainId);
  ...
  async getTokenPrice(network, addresses, options = {}) {
    ...
    const result = await this.coinGeckoApi.getTokenPrice(network, addresses, options);
    
    return {
      message: 'Token prices retrieved successfully',
      data: result,
      summary: `Retrieved prices for ${addresses.split(',').length} token(s) on ${network} network`
    };
  }
  ...
  async getTrendingPoolsByNetwork(network, options = {}) {
    ...
    const result = await this.coinGeckoApi.getTrendingPoolsByNetwork(network, options);
  ...
  async getMultiplePoolsData(network, addresses, options = {}) {
    ...
    const result = await this.coinGeckoApi.getMultiplePoolsData(network, addresses, options);
    
    return {
      message: 'Multiple pools data retrieved successfully',
      data: result,
      summary: `Retrieved data for ${addresses.split(',').length} pool(s) on ${network}`
    };
  }
  ...
  async getTopPoolsByDex(network, dex, options = {}) {
    ...
    const result = await this.coinGeckoApi.getTopPoolsByDex(network, dex, options);
  ...
  async getNewPools(options = {}) {
    ...
    return {
      ...
      data: result,
      summary: `Found ${result.data?.length || 0} new pools across all networks`
    };
  }
  ...
  async searchPools(query, options = {}) {
    ...
    return {
      ...
      data: result,
      summary: `Found ${result.data?.length || 0} pools matching "${query}"${options.network ? ` on ${options.network}` : ''}`
    };
  }
  ...
  async getTopPoolsByToken(network, tokenAddress, options = {}) {
    ...
    const result = await this.coinGeckoApi.getTopPoolsByToken(network, tokenAddress, options);
    
    return {
      message: `Top pools for token ${tokenAddress} on ${network} retrieved successfully`,
      data: result,
      summary: `Found ${result.data?.length || 0} pools for token ${tokenAddress}`,
  ...
  async getTokenData(network, address, options = {}) {
    ...
    const result = await this.coinGeckoApi.getTokenData(network, address, options);
    
    return {
      message: `Token data for ${address} on ${network} retrieved successfully`,
      data: result,
      summary: `Retrieved data for token ${result.data?.attributes?.symbol || address}`,
  ...
  async getMultipleTokensData(network, addresses, options = {}) {
    ...
    const result = await this.coinGeckoApi.getMultipleTokensData(network, addresses, options);
    
    return {
      message: 'Multiple tokens data retrieved successfully',
      data: result,
      summary: `Retrieved data for ${addresses.split(',').length} token(s) on ${network}`,
      includes: options.include ? options.include.split(',') : []
    };
  }
  ...
  async getTokenInfo(network, address) {
    ...
    return {
      ...
      data: result,
      summary: `Retrieved detailed info for token ${result.data?.attributes?.symbol || address}`,
  ...
  async getRecentlyUpdatedTokens(options = {}) {
    ...
    return {
      ...
      data: result,
      summary: `Found ${result.data?.length || 0} recently updated tokens${options.network ? ` on ${options.network}` : ' across all networks'}`,
      includes: options.include ? options.include.split(',') : []
    };
  }
  ...
  async getPoolOHLCV(network, poolAddress, timeframe, options = {}) {
    if (!network || !poolAddress || !timeframe) {
      throw new Error('Missing required parameters: network, poolAddress, timeframe');
    }

    const result = await this.coinGeckoApi.getPoolOHLCV(network, poolAddress, timeframe, options);
  ...
  async getPoolTrades(network, poolAddress, options = {}) {
    ...
    const result = await this.coinGeckoApi.getPoolTrades(network, poolAddress, options);
  ...
  async getGaslessPrice(params) {
    ...
    const result = await this.zeroExApi.getGaslessPrice(params);
  ...
  async getGaslessQuote(params) {
    ...
    const result = await this.zeroExApi.getGaslessQuote(quoteParams);
  ...
  async submitGaslessSwap(params) {
    ...
    if (!this.userPrivateKey) {
      throw new Error('User private key is required for gasless swap execution');
    }
    ...
    try {
      ...
      const chainId = quoteData.trade?.eip712?.domain?.chainId || params.chainId;
      if (!chainId) {
      ...
      if (quoteData.approval) {
        ...
        const signedApproval = await this.blockchain.signGaslessApproval(quoteData.approval);
      ...
      const signedTrade = await this.blockchain.signGaslessTrade(quoteData.trade);
      ...
      const result = await this.zeroExApi.submitGaslessSwap(submissionData);
  ...
  async getGaslessStatus(params) {
    ...
    const result = await this.zeroExApi.getGaslessStatus(tradeHash, chainId);
  ...
  async getGaslessChains() {
    const result = await this.zeroExApi.getGaslessChains();
    
    return {
      message: 'Gasless supported chains retrieved successfully',
      data: result,
      summary: `Found ${result.chains?.length || 0} chains supporting gasless swaps`,
  ...
  async getGaslessApprovalTokens(params = {}) {
    ...
    const result = await this.zeroExApi.getGaslessApprovalTokens(chainId);
    
    return {
      message: 'Gasless approval tokens retrieved successfully',
      data: result,
      summary: `Found ${result.tokens?.length || 0} tokens supporting gasless approvals on chain ${chainId}`,
  ...
  async getPortfolioTokens(params) {
    const { addresses, withMetadata, withPrices, includeNativeTokens } = params;
    
    if (!addresses || !Array.isArray(addresses)) {
      throw new Error('addresses is required and must be an array');
    }

    const result = await this.zeroExApi.getPortfolioTokens(addresses, {
      withMetadata,
      withPrices,
      includeNativeTokens
    });
    ...
    return {
      ...
      data: result,
      summary: `Retrieved tokens for ${addresses.length} address(es) across ${addresses.reduce((total, addr) => total + addr.networks.length, 0)} network(s)`,
      options: {
  ...
  async getPortfolioBalances(params) {
    const { addresses, includeNativeTokens } = params;
    
    if (!addresses || !Array.isArray(addresses)) {
      throw new Error('addresses is required and must be an array');
    }

    const result = await this.zeroExApi.getPortfolioBalances(addresses, {
      includeNativeTokens
    });
    ...
    return {
      ...
      data: result,
      summary: `Retrieved balances for ${addresses.length} address(es) across ${addresses.reduce((total, addr) => total + addr.networks.length, 0)} network(s)`,
  ...
  async getPortfolioTransactions(params) {
    const { addresses, before, after, limit } = params;
    ...
    if (addresses.length !== 1) {
      throw new Error('Transactions API currently supports only 1 address (BETA limitation)');
    }

    const result = await this.zeroExApi.getPortfolioTransactions(addresses, {
      before,
      after,
      limit
    });
    ...
    return {
      ...
      data: result,
      summary: `Retrieved ${result.transactions?.length || 0} transactions for address ${addresses[0].address}`,
      pagination: {
        limit: limit || 25,
      ...
      beta: {
        limitations: 'Currently supports 1 address and max 2 networks (eth-mainnet, base-mainnet)',
